...
function extractTokenFromCookies(cookies: string, indexOfTokenKey: number): string {
...
export default function handleRoutes() {
    ...
    //
    async function goOut(route: "logout" | "delete-account", req: Request, res: Response) {
        const cookies = req.headers.cookie;
        if (cookies) {
            const indexOFAccessTokenKey = cookies.indexOf("access_token");
            if (indexOFAccessTokenKey !== -1) {
