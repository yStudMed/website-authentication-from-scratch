...
export async function addNewUser(email: string, passwordHash: string) {
    const result = await pool.query(`
...
//
export async function insertNewAccessToken(userId: number) {
...
export async function accessTokenlookUp(token: string) {
    const result = await pool.query(`
...
export async function signUpSessionTokenLookup(sessionToken: string) {
    const result = await pool.query(`
